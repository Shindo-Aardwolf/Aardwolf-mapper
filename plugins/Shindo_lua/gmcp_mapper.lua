...
function map_list_rooms (SearchData)
	...
	local SQLQuery =
	"SELECT rooms_lookup.uid AS uid, rooms_lookup.name AS name, area "..
	"FROM (SELECT uid, name FROM rooms_lookup WHERE name LIKE %s) "..
...
-- map_list_rooms function contributed by Spartacus
function map_list_exact_rooms (SearchData)
	-- ok, so if I want to lookup a room in my db, I don't want it only if the mapper can find
	-- a sw in a certain # of rooms. if it is in the db, I want its vnum and area, so that I can
	-- figure out how to get there if the mapper does not know.
	-- Now has the added functionality that it can return a table of room numbers if requested to.
	...
	local SQLQuery =
	"SELECT rooms_lookup.uid AS uid, rooms_lookup.name AS name, area "..
	"FROM (SELECT uid, name FROM rooms_lookup WHERE name = %s) "..
	"AS rooms_lookup JOIN rooms ON rooms_lookup.uid = rooms.uid ORDER BY area LIMIT 101;"
	for row in dbnrowsWRAPPER(string.format (SQLQuery, fixsql (name))) do
		if count < 101 then
			if ReturnList  == 0 then
...
-- map_list_rooms function contributed by Spartacus
function map_list_exact_rooms_extended (SearchData, ReturnAsList, SearchByArea)
	-- ok, so if I want to lookup a room in my db, I don't want it only if the mapper can find
	-- a sw in a certain # of rooms. if it is in the db, I want its vnum and area, so that I can
	-- figure out how to get there if the mapper does not know.
	-- Now has the added functionality that it can return a table of room numbers if requested to.
	...
	local SQLQuery = "SELECT rooms_lookup.uid as uid, rooms_lookup.name as name, "..
	"area FROM (SELECT uid, name FROM rooms_lookup WHERE name = %s) "..
	"AS rooms_lookup JOIN rooms ON rooms_lookup.uid = rooms.uid ORDER BY area LIMIT 101;"
	for row in dbnrowsWRAPPER(string.format (SQLQuery, fixsql (name))) do
		if SearchArea == "" then
			if count < 101 then
	...
	return {}
end -- map_list_exact_rooms_extended
...
function populate_exact_room_list(RoomName)
	RoomListTable = map_list_exact_rooms("1 "..RoomName)
	Note("+------------------------------ START OF SEARCH -------------------------------+\n")
	for count, RoomInfo in ipairs(RoomListTable) do
		Note(string.format("%03d, ( %5s ) %-40s is in \"%s\"\n",
		count, RoomInfo["uid"], RoomInfo["name"], RoomInfo["area"]))
...
function populate_exact_room_list_with_area(SearchData)
	--SearchData == "areaname roomname"
	local _, _, SearchArea, RoomName = string.find(SearchData,"^(%w+)%s(.*)$")
	SearchArea = string.lower(SearchArea)
	if SearchArea == "here" then
	...
	RoomListTable = map_list_exact_rooms_extended(RoomName, 1, SearchArea)
	Note("+------------------------------ START OF SEARCH -------------------------------+\n")
	for count, RoomInfo in ipairs(RoomListTable) do
		Note(string.format("%03d, ( %5s ) %-40s is in \"%s\"\n",
		count, RoomInfo["uid"], RoomInfo["name"], RoomInfo["area"]))
...
function quickRunTo(AreaName)  --ZZZ
	destination = string.lower(AreaName)
	if (destination == "ft2") then destination = "ftii" end -- HACK for ftii (allows one to 'xrt ft2' as well as 'xrt ftii')
	for LongName, AreaData in pairs(AreaByLongName) do
		if (string.match(AreaData[1], ("^"..destination)) ~= nil and AreaData[5] ~= "") then
			Note(string.format("%sRunning to %s%s.%s\n", dgreen, bgreen, LongName, dwhite))
			map_goto(AreaData[5])
			return
		elseif (string.match(AreaData[1], ("^"..destination)) ~= nil and AreaData[5] == "") then
			Note(string.format("%s is HIDDEN!! if you know the uid add it to AreaArray.lua.\n", destination))
		end
	end
	Note("No area found to run to, try a different keyword.\n")
end
...
RegisterSpecialCommand("xrt", "quickRunTo")
...
RegisterSpecialCommand("MapperPopulateRoomListExact", "populate_exact_room_list")
RegisterSpecialCommand("MapperPopulateRoomListAreaExact", "populate_exact_room_list_with_area")
